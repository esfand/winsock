...
void CTestClientDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_REC, m_wndRec);
	DDX_Control(pDX, IDC_IPADDRESS, m_ipAddress);
	DDX_Text(pDX, IDC_REPS, m_strRep);
	DDX_Text(pDX, IDC_DATAOUT, m_strDataOut);
	//}}AFX_DATA_MAP
}
...
BEGIN_MESSAGE_MAP(CTestClientDlg, CDialog)
	...
	ON_MESSAGE(WM_REC_MSG, OnRecMsg)
...
void CTestClientDlg::OnConnect() 
{
	...
	m_socket.Init(str, 999, m_hWnd);
}

void CTestClientDlg::OnSend() 
{
	UpdateData(TRUE);
	int nCnt = atoi(m_strRep);
	for (int n = 0; n < nCnt ; n++)
		m_socket.SendData(m_strDataOut);
	
}



LRESULT CTestClientDlg::OnRecMsg(WPARAM wParam, LPARAM lParam)
{
	char* pString = reinterpret_cast<char*>(lParam);
	m_wndRec.SetWindowText(pString);
	delete [] pString;
...
LRESULT CTestClientDlg::OnStatusMsg(WPARAM wParam, LPARAM lParam)
{
	char* pString = reinterpret_cast<char*>(lParam);

	m_list.AddString(pString);
	delete [] pString;
...
void CTestClientDlg::OnFlood()
{
	m_bFlood = !m_bFlood;
	if (m_bFlood)
	{
		UpdateData(TRUE);
		SetTimer(IDT_TIMER, 10, NULL);
	}
	else
	{
		KillTimer(IDT_TIMER);
