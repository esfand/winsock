...
int main(int /*argc*/, char * /*argv[ ]*/)
{
   try
   {
      ...
      CSocketServer server1(
         lockFactory,
         0,                            // number of threads in the io thread pool (0 = 2 x processors)
         "Welcome to echo server 1\r\n",
      ...
      CSocketServer server2(
         lockFactory,
         pool,                         // share io threads with server 3
         "Welcome to echo server 2\r\n",
         INADDR_ANY,                   // address to listen on
      ...
      CSocketServer server3(
         lockFactory,
         pool,                         // share io threads with server 2
         "Welcome to echo server 3\r\n",
         INADDR_ANY,                   // address to listen on
         5003,                         // port to listen on
         10,                           // max number of sockets to keep in the pool
         10,                           // max number of buffers to keep in the pool
         1024);                        // buffer size 
      ...
      server3.StartAcceptingConnections();
      ...
      while (!done)
      {
         ...
         else if (waitResult == WAIT_OBJECT_0 + 1)
         {
            if (accepting)
            {
               ...
               server3.StopAcceptingConnections();
            }
            else
            {
               server1.StartAcceptingConnections();
      ...
      server3.WaitForShutdownToComplete();
   }
